-- ============================================================================
-- N-LIC CRM 初期スキーマ
-- 02_phase1_database.md §1〜19 に完全準拠
-- ----------------------------------------------------------------------------
-- 全 16 テーブル + ビュー + RLS + ヘルパー関数 + トリガーを 1 マイグレーションで構築。
-- 保険業法対応データ (intention_records / contact_histories) は DB レベルで DELETE 禁止。
-- ============================================================================


-- ─────────────────────────────────────────────────────────────────────────
-- 1. 拡張機能
-- Supabase は uuid-ossp を `extensions` schema に置くため public からは呼びにくい。
-- 代わりに Postgres 13+ 組込みの gen_random_uuid() を使うので uuid-ossp は不要。
-- ─────────────────────────────────────────────────────────────────────────
...
CREATE TABLE tenants (
  ...
  --   "lark": {
  --     "sso_enabled": true,
  --     "base":     { "base_id": "...", "sync_customers": true, "sync_contracts": true, "sync_opportunities": true },
  --     "calendar": { "calendar_id": "...", "sync_tasks": true, "sync_events": true },
  ...
  --   }
  -- }
  -- App ID / App Secret は env で管理し、ここには保存しない (Lark Rules §6)
...
-- ─────────────────────────────────────────────────────────────────────────
-- 15. カレンダーイベント (start_at / end_at で統一)
-- ─────────────────────────────────────────────────────────────────────────
...
-- 19-2. tenants / user_profiles
CREATE POLICY tenants_select ON tenants
  FOR SELECT USING (id = current_user_tenant_id());
CREATE POLICY tenants_update ON tenants
  FOR UPDATE USING (
    id = current_user_tenant_id() AND current_user_role() = 'admin'
  );
...
-- 19-4. contracts (customers と同パターン)
...
-- 19-8. 子テーブル (テナント単位 SELECT/INSERT/UPDATE/DELETE 許可、テナント分離のみ)
...
CREATE POLICY contract_riders_tenant ON contract_riders
  FOR ALL USING (tenant_id = current_user_tenant_id())
  WITH CHECK (tenant_id = current_user_tenant_id());

CREATE POLICY opportunity_activities_tenant ON opportunity_activities
  FOR ALL USING (tenant_id = current_user_tenant_id())
  WITH CHECK (tenant_id = current_user_tenant_id());

CREATE POLICY opportunity_suitability_tenant ON opportunity_suitability
  FOR ALL USING (tenant_id = current_user_tenant_id())
  WITH CHECK (tenant_id = current_user_tenant_id());

-- 19-9. 保険業法対応データ: SELECT/INSERT/UPDATE は通常、DELETE のみ全拒否
CREATE POLICY contact_histories_select ON contact_histories
  FOR SELECT USING (tenant_id = current_user_tenant_id());
...
CREATE POLICY intention_records_select ON intention_records
  FOR SELECT USING (tenant_id = current_user_tenant_id());
...
CREATE POLICY intention_products_tenant ON intention_products
  FOR ALL USING (tenant_id = current_user_tenant_id())
  WITH CHECK (tenant_id = current_user_tenant_id());
...
CREATE POLICY settlements_select ON settlements
  FOR SELECT USING (tenant_id = current_user_tenant_id());
CREATE POLICY settlements_modify ON settlements
  FOR ALL USING (
    tenant_id = current_user_tenant_id()
...
CREATE POLICY settlement_imports_select ON settlement_imports
  FOR SELECT USING (tenant_id = current_user_tenant_id());
CREATE POLICY settlement_imports_modify ON settlement_imports
  FOR ALL USING (
    tenant_id = current_user_tenant_id()
    AND current_user_role() IN ('admin', 'staff')
  )
...
-- 19-11. MDRT
CREATE POLICY mdrt_targets_select ON mdrt_targets
  FOR SELECT USING (tenant_id = current_user_tenant_id());
CREATE POLICY mdrt_targets_admin ON mdrt_targets
  FOR ALL USING (
    tenant_id = current_user_tenant_id() AND current_user_role() = 'admin'
  )
  WITH CHECK (
...
CREATE POLICY mdrt_performances_select ON mdrt_performances
  FOR SELECT USING (tenant_id = current_user_tenant_id());
CREATE POLICY mdrt_performances_modify ON mdrt_performances
  FOR ALL USING (
    tenant_id = current_user_tenant_id()
    AND current_user_role() IN ('admin', 'staff')
  )
...
-- 19-12. notification_logs / audit_logs (書込はサーバー Service Role 専用)
