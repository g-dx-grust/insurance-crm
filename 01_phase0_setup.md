...
```bash
pnpm dlx create-next-app@latest n-lic-crm \
  --typescript \
  --tailwind \
  --eslint \
  --app \
...
2. プロジェクト名: `n-lic-crm`、リージョン: `Northeast Asia (Tokyo)`
...
export default async function RootLayout({
  ...
  const cookieStore = await cookies()
  const theme =
    (cookieStore.get('nlic_theme')?.value as 'light' | 'dark' | undefined) ?? 'light'
  const fontScale =
    (cookieStore.get('nlic_font_scale')?.value as FontScaleKey | undefined) ?? 'normal'
